#!/usr/bin/env python3
"""In-container script runner for OoLu sandboxes.

Baked into the sandbox image at /opt/oolu/entrypoint.py. The host backend runs
Phase B as:

    python /opt/oolu/entrypoint.py /sandbox/user_script.py
...
``OOLU_SANDBOX_DIR`` lets tests point the runner at a temp directory; in the image
...
_SANDBOX = os.environ.get("OOLU_SANDBOX_DIR", "/sandbox")


def main(argv: list[str]) -> int:
    # The shim (_oolu_runtime.py) and the user script live in the sandbox dir.
    if _SANDBOX not in sys.path:
        sys.path.insert(0, _SANDBOX)

    try:
        import _oolu_runtime as rt
    except Exception:  # noqa: BLE001 - shim missing => cannot honor the contract at all
        sys.stderr.write("oolu entrypoint: result shim not found in sandbox dir\n")
        return 3
