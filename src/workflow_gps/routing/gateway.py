...
_TRUNCATED_FENCE_RE = re.compile(r"```[ \t]*(?:python|py|python3)?[ \t]*\r?\n(.*)$", re.DOTALL)
...
def extract_script(text: str | None) -> str | None:
    ...
    if blocks:
        python_blocks = [body.strip() for lang, body in blocks
...
class Gateway(Protocol):
    @property
    def name(self) -> str: ...
    def complete(self, decision: RoutingDecision, prompt: AssembledPrompt) -> SynthesisResult: ...
    def close(self) -> None: ...
...
class LiteLLMGateway:
    ...
    def __init__(self, *, request_timeout: float = 120.0, drop_unsupported_params: bool = True):
        try:
            import litellm
        except ImportError as exc:
            raise GatewayUnavailable("litellm not installed (`pip install litellm`)") from exc
    ...
    def complete(self, decision: RoutingDecision, prompt: AssembledPrompt) -> SynthesisResult:
        ...
        try:
            ...
            raise GatewayError(f"completion failed via {kwargs.get('model')}: {exc}") from exc
...
class FakeGateway:
    ...
    def complete(self, decision: RoutingDecision, prompt: AssembledPrompt) -> SynthesisResult:
        ...
        if not self._completions:
            raise GatewayError(f"fake gateway '{self._name}' has no scripted completions left")
        nxt = self._completions.pop(0)
        if isinstance(nxt, Exception):
            raise nxt
        text = nxt(decision, prompt) if callable(nxt) else nxt
        return SynthesisResult(
            raw_text=text, script=extract_script(text), model=decision.model,
            tier=decision.tier, finish_reason="stop",
        )
