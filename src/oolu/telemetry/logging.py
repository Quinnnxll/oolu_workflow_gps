...
Three concerns, all degrading gracefully when ``rich`` is absent:
  ...
  * ``configure_logging`` installs a rich handler on the ``oolu`` logger so the
...
def render_result(result, *, console=None) -> None:
    ...
    try:
        ...
        (console or Console()).print(
            Panel(table, title=f"OoLu · {title}", expand=False)
        )
    except ImportError:
        line = " | ".join(f"{k}={v}" for k, v in rows)
        print(f"OoLu [{'OK' if success else 'FAIL'}] {line}")
