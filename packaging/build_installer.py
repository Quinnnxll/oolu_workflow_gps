"""Build the single-file Workflow-GPS shell app for THIS platform.
...
def main() -> int:
    _ensure("workflow_gps", "-e", f"{ROOT}[serve]")
    ...
    subprocess.check_call(
        [
            ...
            str(HERE / "wfgps_shell.spec"),
        ]
    )
    artifact = next((ROOT / "dist").glob("WorkflowGPS-Shell*"))
    print(f"\nBuilt: {artifact}")
