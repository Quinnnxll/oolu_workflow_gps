...
tests pin the remedies: `wfgps doctor` diagnoses each trap with its exact
fix, `wfgps run` preflights the same traps before any engine machinery can
...
def test_doctor_names_the_engine_extra_when_missing(monkeypatch):
    ...
    assert code == 0, report
    assert 'pip install "workflow-gps[engine]"' in report
    assert "needed only for `wfgps run`" in report
...
# --------------------------------------------------------------------------- #
# `wfgps run` preflight: the audit's tracebacks become one-line fixes.         #
# --------------------------------------------------------------------------- #
def test_run_preflight_catches_the_missing_engine(monkeypatch, capsys):
    ...
    assert 'pip install "workflow-gps[engine]"' in err
...
def test_run_preflight_catches_the_dead_model_server(monkeypatch, capsys):
    ...
    assert "localhost:8000" in err and "wfgps doctor" in err
...
def test_bare_file_invocation_gives_directions_not_an_import_traceback():
    proc = subprocess.run(
        [sys.executable, str(ROOT / "src" / "workflow_gps" / "cli.py"), "--help"],
        capture_output=True,
        text=True,
        cwd=ROOT,
    )
    assert proc.returncode == 2
    assert "setup.bat" in proc.stderr and "python -m workflow_gps.cli" in proc.stderr
    assert "Traceback" not in proc.stderr


def test_uvicorn_asgi_app_is_a_signpost_not_an_attribute_error():
    from workflow_gps.gateway.asgi import app  # the exact target the audit hit
    ...
    assert "wfgps desktop" in text and "setup.bat" in text
...
def test_readme_documents_the_doctor():
    ...
    assert "wfgps doctor" in readme
    assert 'pip install "workflow-gps[engine]"' in readme
