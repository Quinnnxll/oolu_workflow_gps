...
`wfgps host` serves the full multi-tenant gateway with this install as its
...
from workflow_gps.identity.accounts import LOCKOUT_THRESHOLD
...
def _service(clock=None):
    ...
    signer = Hs256Signer(secret=SECRET, issuer="wfgps-local", audience="wfgps")
...
def _run_host(monkeypatch, tmp_path, *, secret=SECRET, admin_password="first-pass"):
    ...
    for name, value in (
        ("WFGPS_HOST_SECRET", secret),
        ("WFGPS_ADMIN_PASSWORD", admin_password),
    ):
        if value is None:
...
def test_wfgps_host_serves_the_gateway_asgi(monkeypatch, tmp_path):
...
def test_wfgps_host_generates_credentials_once_when_unset(monkeypatch, tmp_path):
...
def test_wfgps_host_refuses_a_short_secret(monkeypatch, tmp_path, capsys):
